...
We found that tokens in the global space were more similar to themselves within the year than between years, while identical tokens in unaligned models were completely distinct.
Local distances were unaffected by alignment, as token-neighbor distances remained unchanged.
Figures {@fig:word2vec_alignment}A-D show our results.
...
Before each changepoint, the closest neighbors for 'SARS' were consideredmiscellaneous.
After the even occurs, the neighbors for 'SARS' were consistent with the acronym for Severe Acute Respiratory Syndrome (Tables {@tbl:sars_neighbor_table_one} and {@tbl:sars_neighbor_table_two}).
