...
Pubtator Central is an open-access resource containing annotated abstracts and full-text [@doi:10.1093/nar/gkz389].
Every generated annotation used entity recognition systems to label specific words and phrases to represent biomedical concepts.
These systems consisted of TaggerOne [@doi:10.1093/bioinformatics/btw343] to tag diseases, chemicals, and cell line entities, GNormPlus [@doi:10.1155/2015/918710] to tag 
...
This resource contains documents that date back to the pre-1800s to the year 2021; however, due to the low sample size, we only used documents published from 2000 to 2021.
Since our initial download, this resource provided a new update containing more documents for 2021. 
Due to this update, we also downloaded a later version on March 09th, 2022, and merged both versions to have sufficient data for this project.
...
Then, we used spacy to brake down every sentence into individual tokens and normalized each token to its root form via lemmatization.
...
To prevent duplication bias, we filtered every preprint to be its most recent version and sorted each preprint into its respective posted year.
...
Once tagged, we used spacy to preprocess every preprint in the same medium as described in our Pubtator Central section.
...
Despite the power of these word2vec models, it has been shown that they have stability issues within year and across years [@arxiv:1804.09692; @doi:10.1007/978-3-030-03991-2_73; @doi:10.1162/tacl_a_00008; @doi:10.18653/v1/S18-2019].
To correct this instability, we took an intra-year and inter-year approach.
...
Regarding the intra-year distance, we took the combination of every model pair trained in the same year.
...
For the inter-year distance, we took the cartesian product of every model between two years and calculated the distance between tokens in the same medium as the intra-year distance.
...
Following time series construction, the final step is to perform change point detection, which is a process that uses statistical techniques to detect abnormalities within a given time series.
...
A changepoint has occurred if the sum is greater than a threshold.
We devised a threshold by calculating the 99th percentile on every generated timepoint.
Then, we ran the CUSUM algorithm using our calculated threshold,  a drift of 0, and default settings for all other parameters.
